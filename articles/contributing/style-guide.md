...
Jeśli operacja lub funkcja nie jest przeznaczona do użytku bezpośredniego, ale powinna być używana przez pasujące zadanie, które działa przez częściową aplikację, należy rozważyć użycie nazwy zaczynającej się od do możliwego do odebrania `_` .
...
|   | Nazwa | Opis |
|---|------|-------------|
| ☒ | <s>`ApplyDecomposedOperation_`</s> | Znak podkreślenia `_` nie powinien występować na końcu nazwy. |
