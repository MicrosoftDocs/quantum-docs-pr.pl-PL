...
Zamiast przypisywania poszczególnych $Nów _e $ Electrons do określonego stanu (lub orbity), druga podziału śledzi każdą wartość w każdym z nich i przechowuje dane, czy w każdym z nich jest wystawcy, czy w tym samym czasie automatycznie zapewnia właściwości symetrii odpowiadająca funkcja Wave.
...
Oznacza to, że prawidłowe Stany podstawowe mają postać $ \ psi_ {0, \uparrow}, \ldots, \ psi_ {N-1, \uparrow}, \ psi_ {0, \downarrow}, \ldots, \ psi_ {N-1, \downarrow} $ WHERE $ \uparrow $ i $ \downarrow $ są etykietami, które określają dwa eigenstates stopnia pokrętła ruchów.
...
Oznacza to, że możemy formalnie myśleć o podstawie dla składnika "Wave" i "przestrzenny" tej funkcji, jako $ \ psi_{0} \cdots \ psi_ {2N-1} $, gdzie każdy indeks jest teraz wyliczeniem $ (j, \sigma) $.
...
Oznacza to, że możemy napisać dwa stany prawne dla $ \ psi_1 $ as \begin{Equation} \ psi_1 \rightarrow \begin{Cases} \ket{0}_1 & \Text{if $ \ psi_1 $ nie jest zajmowane,} \\\
...
Oznacza to, że $ $ a ^ \ dagger_2 a ^ \ dagger_1 \ket{0} =-a ^ \ dagger_1 a ^ \ dagger_2 \ket{0}.
...
\begin{Equation} \hat{H} = \sum\_{pq} H\_{pq} a ^ \dagger\_p a\_q + \frac{1}{2}\sum\_{pqrs} H\_{pqrs} a ^ \dagger\_p a ^ \dagger\_p a\_RA\_s + H\_{\textrm NUC}, \label{EQ: totalHam} \end{Equation}, gdzie $h\_{\textrm NUC} $ jest energią jądrową (która jest stałą w ramach przybliżania ponoszonego przez Oppenheimer) i
...
Warunki $h\_{pq} $ są określane mianem jednoskładnikowych, ponieważ wszystkie te warunki obejmują tylko jedną electronsę i podobnie $h\_{pqrs} $ to dwie części-elektronów.
...
Na przykład w przypadku, gdy $p, q, r, s $ są oddzielnymi indeksami, firma Microsoft może używać zasad ochrony antyszpiegowskiej, aby pokazać, że: $ $ a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} =-a ^ \dagger\_{q} a ^ \dagger\_{p} a\_{r} a\_{s} =-a ^ \dagger\_{p} a ^ \dagger\_{q}\_{s} a\_{r} = a ^ \dagger\_{q} a ^ \dagger\_{p} a\_{s} a\_{r}.
$$

Co więcej, jako $H $ hermitian, każdy operator fermionic hermitian, powiedz $h\_{pqrs} a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} $, ma hermitian koniugat, który jest również znaleziony w $H $. W celu jednoznacznego indeksowania grup warunków scharakteryzowanych przez te symmetries definiujemy porządkowanie kanoniczne na indeksach $ (i\_1, \cdots, i\_n, j\_1, \cdots, j\_m) $ dowolnej sekwencji $n + m $ fermionic operatory $a ^ \dagger\_{i\_1} \cdots {\dagger\_{i\_n} a\_{j\_1} \cdots\_{j\_m} :
...
W tym porządku kanonicznym fermionic hamiltonian może być wyrażony jako $ $ \begin{align} H = \sum\_{(p, q) \In S\_{1,1}} H "\_{pq} \frac{a ^ \dagger\_{p} a\_{q} + a ^ \dagger\_{q} a\_{p}}{2}+ \sum\_{(p, p, r, S) \In S\_{2,2}} H"\_{pqrs} \frac{a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} + a ^ \dagger\_{s} a ^ \ Dagger\_{r}\_{q} a\_{p}}{2}, \end{align} $ $ z odpowiednio przystosowanymi integralnymi dwoma i dwukolorowymi $h "\_{pq} $ i $h"\_{pqrs} $.
