...
<span data-ttu-id="338ac-109">Oprócz wyliczania wszystkich całkowitych częściowych, które są identycznie takie same, lista wszystkich indeksów kręgowych zawartych w hamiltonian reprezentowanej przez `OrbitalIntegral` może być generowana w następujący sposób.</span><span class="sxs-lookup"><span data-stu-id="338ac-109">In addition to enumerating over all orbital integrals that are numerically identical, a list of all spin-orbital indices contained in the Hamiltonian represented by an `OrbitalIntegral` may be generated as follows.</span></span>
```csharp
...
```
## <a name="constructing-fermionic-hamiltonians-from-molecular-integrals"></a><span data-ttu-id="338ac-110">Konstruowanie Fermionic Hamiltonians z całk molekularnych</span><span class="sxs-lookup"><span data-stu-id="338ac-110">Constructing Fermionic Hamiltonians from Molecular Integrals</span></span>

<span data-ttu-id="338ac-111">Zamiast konstruowania Fermionic hamiltonian przez dodanie `FermionTerm`s, wszystkie warunki odpowiadające każdej liczbie całkowitej mogą zostać dodane automatycznie.</span><span class="sxs-lookup"><span data-stu-id="338ac-111">Rather than constructing a Fermionic Hamiltonian by adding `FermionTerm`s, all terms corresponding to each orbital integral may be added automatically.</span></span>
<span data-ttu-id="338ac-112">Na przykład poniższy kod automatycznie wylicza wszystkie permutacje symmetries i porządkuje warunki w kolejności kanonicznej:</span><span class="sxs-lookup"><span data-stu-id="338ac-112">For example, the following code automatically enumerates over all permutational symmetries and orders the terms in canonical order:</span></span> 
```csharp
