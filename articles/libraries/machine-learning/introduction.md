...
Kodowanie Quantum i przetwarzanie informacji jest zaawansowaną alternatywą dla klasycznych klasyfikatorów opartych na usłudze Machine Learning, w szczególności, kodowania dane w rejestrach Quantum, które są zwięzłe względem liczby funkcji, systematycznie wykorzystują Quantum Entanglement jako zasób obliczeniowy i Zastosuj pomiar Quantum dla wnioskowania klas.
...
Klasyfikacja to zadanie nadzorowanej uczenia maszynowego, w którym celem jest wywnioskowanie etykiet klas $\{y_1, y_2, \ldots, y_d\}$ niektórych próbek danych. "Zestaw danych szkoleniowych" to kolekcja przykładów $ \mathcal{D} =\{(x, y)} $ ze znanymi wstępnie przypisanymi etykietami. Tutaj $x $ to przykład danych, a $y $ to jego znana etykieta o nazwie "etykieta szkoleniowa".
