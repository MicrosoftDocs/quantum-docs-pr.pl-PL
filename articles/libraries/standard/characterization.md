...
Oznacza to, że zamiast przygotował $n $-qubit Register, aby zawierał binarną reprezentację fazy w ramach szacowania fazy Quantum, możemy wyświetlić oszacowanie fazy jako proces, za pomocą którego Agent *klasyczny* pouczy właściwości systemu Quantum przez pomiarów.
...
W przypadku tej definicji, każdy krok iteracji fazy szacowania jest realizowany przez przygotowanie Auxillary qubit w stanie $ \ket{+} $ wraz z początkowym stanem $ \ket{\phi} $ przyjętym przez nas jest [eigenvector](xref:microsoft.quantum.concepts.matrix-advanced) of $U (m) $, tj. $U (m) \ket{\phi} = e ^ {im\phi} \ket{\phi} $.  
...
Podkreślamy to, krótko opisując ocenę fazy iteracyjnej na poziomie teoretycznym w formalnym wykorzystaniu parametrów bayesowskie, przed kontynuowaniem opisywania algorytmu statystycznego podanego w Q # Canon do rozwiązywania tego klasycznego wnioskowania związane.

### <a name="iterative-phase-estimation-without-eigenstates"></a>Szacowanie fazy iteracyjnej bez Eigenstates ###

Jeśli podany jest stan wejściowy, który nie jest eigenstate, co oznacza, że jeśli $U (m) \ket{\phi\_j} = e ^ {im\phi\_j} $, wówczas proces szacowania fazy nie jest niejednoznaczny, a stan Quantum w ramach jednego eigenstatea energii.  Eigenstate, do którego ostatecznie jest zbieżny, jest eigenstate, który najprawdopodobniej wygenerował obserwowane `Result`.

W odniesieniu do jednego kroku środowiska uruchomieniowego na stanie \begin{align} \sum_j \sqrt{\Pr (\phi\_j)} \ket{\phi\_j} \mapsto \sum\_j\frac {\ sqrt {\ PR (\phi\_j)} \sqrt{\Pr (\text{Result} | \ Fi\_j)} \ket{\phi\_j}} {\sqrt{\Pr (\phi\_j) \sum\_j \Pr (\text{Result} | \phi\_j)}}.
\end{align}, ponieważ ten proces jest powtarzany przez wiele wartości `Result`, eigenstates, które nie mają maksymalnych wartości $ \prod_k\Pr (\text{Result}\_k | \phi\_j) $ zostanie pominięty wykładniczo.
...
Bayesa "theorem dalszej sugestii, że stan, który wynika z oszacowania fazy, jest zapisywana w postaci \begin{align} \frac{\sqrt{\Pr (\phi\_j)} \sqrt{\Pr (\text{Result} | \phi\_j)} \ket{\phi\_j}} {\sqrt{\Pr (\phi\_j) \sum\_j \Pr (\text{Result} | \phi\_j)}} = \sum_j \sqrt{\Pr (\phi\_j | \text{Result})} \ket{\phi\_j}.
w tym miejscu \end{align} $ \Pr (\phi\_j | \text{Result}) $ może być interpretted jako prawdopodobieństwo, że jeden z nich będzie ascribe do każdej hipotezy dotyczącej danego eigenstatesa:
...
> [!TIP]
> Aby uzyskać więcej informacji na temat szacowania fazy bayesowskie w sposób, zobacz przykład [**PhaseEstimation**](https://github.com/Microsoft/Quantum/tree/master/Samples/src/PhaseEstimation) .
...
Należy pamiętać, że $X = \ket{+} \bra{+}-\ket{-}\bra{-}$, tak że $ \ket{+} $ jest jedyną dodatnią eigenstate $X $ odpowiadającym `Zero`.
Prawdopodobieństwo obserwowania `Zero` dla [pomiaru`PauliX`](xref:microsoft.quantum.concepts.pauli) na pierwszym qubitie przy użyciu stanu wejściowego $ \ket{\psi}\ket{\phi} $ jest w tym przypadku \begin{Equation} \Pr (\texttt{zero} | \psi) = \left | \braket{+ | \psi} \right | ^ 2.
...
W konkretnym miejscu \begin{Equation} \Pr (\phi | d) = \frac{\Pr (d | \phi) \Pr (\phi)} {\int \Pr (d | \phi) \Pr (\phi) {\mathrm d} \phi} \Pr (\phi), \end{Equation} WHERE $d \In \\{\texttt{Zero}, \texttt{One}\\} $ to `Result`i gdzie $ \Pr (\phi) $ Opisuje nasze wcześniejsze przekonania o $ \phi $.
...
W dowolnym momencie podczas tej procedury możemy zgłosić fazę $ \hat{\phi} $ wnioskowaną przez klasyczny kontroler jako \begin{Equation} \hat{\phi} \mathrel{: =} \expect [\phi | \Text{data}] = \int \phi \Pr (\phi | \Text{Data}) {\mathrm d} \phi, \end{Equation} WHERE $ \ tekst {Data} $ oznacza cały rekord wszystkich uzyskanych wartości `Result`.
...
> [!TIP]
> Istnieje wiele przykładów, w których jest używane niezawodne szacowanie faz. W przypadku szacowania fazy w przypadku wyodrębnienia energii stanu ziemi w różnych systemach fizycznych należy zapoznać się z przykładem [ **symulacji H2** ](https://github.com/Microsoft/Quantum/tree/master/Samples/src/H2SimulationCmdLine), [przykładem **SimpleIsing** ](https://github.com/Microsoft/Quantum/tree/master/Samples/src/SimpleIsing)i [przykładowym **modelem Hubbard** ](https://github.com/Microsoft/Quantum/tree/master/Samples/src/HubbardSimulation).


### <a name="continuous-oracles"></a>Ciągłe firmy Oracle ###

Możemy również uogólnić z modelu Oracle użytego powyżej, aby umożliwić korzystanie z Oracle w czasie ciągłym, które są modelowane przez typ Canon <xref:microsoft.quantum.oracles.continuousoracle>.
...
Jest to słabsze instrukcje niż w przypadku odrębnego przypadku, ponieważ możemy utworzyć <xref:microsoft.quantum.oracles.discreteoracle>, ograniczając $t = m\,\delta t $ dla niektórych stałych $ \delta t $.
...
Można zastosować dokładnie tę samą analizę omówioną w przypadku [szacowania fazy bayesowskie](#bayesian-phase-estimation) , a funkcja prawdopodobieństwo jest dokładnie taka sama dla tego bardziej ogólnego modelu Oracle: $ $ \Pr (\texttt{zero} | \phi; t, \theta) = \cos ^ 2 \ Left (\frac{t [\phi-\theta]} @no__ t_1_ \right).
...
Na przykład operacja <xref:microsoft.quantum.characterization.robustphaseestimation> omówiona w następnej sekcji ma następujący podpis:

```qsharp
operation RobustPhaseEstimation(bitsPrecision : Int, oracle : DiscreteOracle, eigenstate : Qubit[])  : Double
```

Dane wejściowe `bitsPrecision` są unikatowe dla `RobustPhaseEstimation`, podczas gdy `oracle` i `eigenstate` są wspólne.
Tak więc, jak widać w **H2Sample**, operacja może zaakceptować algorytm szacowania fazy iteracyjnej z danymi wejściowymi `(DiscreteOracle, Qubit[]) => Unit`, aby umożliwić użytkownikowi Określanie algorytmów szacowania dowolnego etapu:
