...
Ten wzorzec można wyrazić za pomocą iteracji i `for` pętle:
...
Jest to jednak znacznie krótsze i łatwiejsze do odczytania, wyrażone w zakresie <xref:microsoft.quantum.canon.applytoeachca> i funkcji manipulowania tablicami, takich jak <xref:microsoft.quantum.arrays.zip>.
...
W Q # firma Microsoft może używać <xref:microsoft.quantum.arrays.indexrange> do reprezentowania tego jako `for` pętli w rejestrze:
...
Następnie możemy użyć tej nowej operacji jako `HAll(register)`, aby zastosować $H \otimes H \otimes \cdots \otimes H $.
...
W szczególności oznacza to, że wywołania do `ApplyToEachCA` mogą pojawić się w operacjach, dla których jest generowana automatycznie.
Podobnie <xref:microsoft.quantum.canon.applytoeachindex> jest przydatne do reprezentowania wzorców `U(0, targets[0]); U(1, targets[1]); ...`formularzy i oferuje wersje dla każdej kombinacji funktory obsługiwane przez dane wejściowe.

> [!TIP]
> `ApplyToEach` jest typu sparametryzowanego, aby można go było używać z operacjami, które pobierają dane wejściowe inne niż `Qubit`.
> Załóżmy na przykład, że `codeBlocks` jest tablicą wartości <xref:microsoft.quantum.errorcorrection.logicalregister>, które należy odzyskać.
> Następnie `ApplyToEach(Recover(code, recoveryFn, _), codeBlocks)` zastosuje `code` kod poprawiający błędy i funkcję odzyskiwania `recoveryFn` niezależnie od siebie.
> Ta wartość jest przechowywana nawet dla klasycznych danych wejściowych: `ApplyToEach(R(_, _, qubit), [(PauliX, PI() / 2.0); (PauliY(), PI() / 3.0]))` zastosuje rotację $ \pi/$2 o $X $, a następnie rotacją $pi/$3 o $Y $.

Program Q # Canon również zapewnia obsługę klasycznych wzorców wyliczenia, które są znane do programowania funkcjonalnego.
Na przykład <xref:microsoft.quantum.arrays.fold> implementuje wzorzec $f (f (f (s\_{\Text{Initial}}, x\_0), x\_1), \dots) $ w celu ograniczenia funkcji na liście.
...
Takie streszczenie umożliwia również bardziej wydajne przestrzeganie obwodów, ponieważ `Controlled` działające na `U(qubit); V(qubit); Adjoint U(qubit);` sekwencji nie muszą działać na poszczególnych `U`ach.
Aby to zobaczyć, niech $c (U) $ jest jednostką, która reprezentuje `Controlled U([control], target)` i let $c (V) $ można zdefiniować w taki sam sposób.
...
Ponieważ operacje kontrolne mogą być kosztowne ogólnie, korzystanie z kontrolowanych wariantów, takich jak `WithC` i `WithCA`, może pomóc w zmniejszeniu liczby funktory kontroli, które należy zastosować.

> [!NOTE]
> Jednym $U z nich jest to, że nie trzeba jeszcze wiedzieć, jak zastosować Funktor `Controlled` do `U`.
> w związku z tym `ApplyWithCA` ma słabsze sygnaturę niż oczekiwano:
> ```qsharp
> ApplyWithCA<'T> : (('T => Unit is Adj),
>     ('T => Unit is Adj + Ctl), 'T) => Unit
> ```

Podobnie <xref:microsoft.quantum.canon.bind> wytwarza operacje, które stosują sekwencję innych operacji z kolei.
...
Ta wartość analogiczna jest precyzyjna przez rozpoznawanie, że operatory jednostkowe odpowiadają dokładnie na skutki uboczne operacji wywołujących, tak że jakakolwiek dekompozycja operatorów jednostkowych w warunkach innych operatorów jednostkowych odpowiada za konstruowanie określonego Wywołanie sekwencji dla klasycznej procedury podrzędnej, która emituje instrukcje do działania jako określone operatory jednostkowe.
W tym widoku `Bind` jest precyzyjnie reprezentacja produktu macierzy, ponieważ `Bind([A, B])(target)` jest równoważne `A(target); B(target);`, co z kolei jest sekwencją wywołującą odpowiadającą $BA $.
...
Na przykład zastosowanie rozwinięcia z najniższym porządkiem daje, że dla wszystkich operatorów $A $ i $B $, które $A = A ^ \dagger $ i $B = B ^ \dagger $, \begin{align} \tag{★} \label{EQ: Trotter-Suzuki-0} \exp (i [A + B] t) = \lim_{n\to\infty} \left (\exp (i t/n) \exp (i B t/n ) \right) ^ n.
...
W konkretnym czasie, w przypadku operacji `U : ((Int, Double, Qubit[]) => Unit is Adj + Ctl` takiej, `A = U(0, _, _)` i `B = U(1, _, _)`, możemy zdefiniować nową operację reprezentującą integralność `U` na czas $t $ przez generowanie sekwencji formularza
...
Sygnatura `DecomposeIntoTimeStepsCA` jest zgodna ze wspólnym wzorcem w języku Q #, gdzie kolekcje, które mogą być tworzone przez tablice lub przez coś, które elementy obliczeniowe na bieżąco są reprezentowane przez krotki, których pierwsze elementy są `Int` wartości wskazujące ich długości.
...
Przejdźmy do sposobu, w jaki <xref:microsoft.quantum.canon.controlledonbitstring> robi to (parametry typu San), a następnie Podzielmy pojedyncze elementy o jeden.
...
Nie wywołamy tej operacji bezpośrednio, ale dodamy `_` na początku nazwy, aby wskazać, że jest to implementacja innej konstrukcji w innym miejscu.
...
Należy pamiętać, że przyjmujemy ciąg bitów, reprezentowany jako tablica `Bool`, którego używamy do określenia warunku, który chcemy zastosować do operacji `oracle`.
Ponieważ ta operacja rzeczywiście wykonuje aplikację bezpośrednio, musimy również przyjąć rejestry kontroli i celu, reprezentowane poniżej jako `Qubit[]`.

Następnie należy zauważyć, że kontrolowanie stanu podstawy obliczeniowej $ \ket{\vec{s}} = \ket{s\_0 s\_1 \dots s\_{n-1}} $ dla ciągu bitowego $ \vec{s} $ może być zrealizowane przez transformowanie $ \ket{\vec{s}} $ do $ \ket{0\cdots 0} $.
W szczególności $ \ket{\vec{s}} = X ^ {s\_0} \otimes X ^ {s\_1} \otimes \cdots \otimes X ^ {s\_{n-1}} \ket{0\cdots 0} $.
Ponieważ $X ^ {\dagger} = X $, oznacza to, że $ \ket{0\dots 0} = X ^ {s\_0} \otimes X ^ {s\_1} \otimes \cdots \otimes X ^ {s\_{n-1}} \ket{\vec{s}} $.
W tym celu możemy zastosować $P = X ^ {s\_0} \otimes X ^ {s\_1} \otimes \cdots \otimes X ^ {s\_{n-1}} $, zastosować `Controlled oracle`i przekształcić z powrotem do $ \vec{s} $.
Ta konstrukcja jest precyzyjna `ApplyWith`, więc piszemy odpowiednio treść nowej operacji:
...
Należy jednak pamiętać, że konieczne jest przekształcenie rejestru *kontrolki* na nasz żądany formularz, więc częściowo stosujemy wewnętrzną operację `(Controlled oracle)` w elemencie *docelowym*.
Spowoduje to pozostawienie `ApplyWith`, które w ten sposób przejdzie do przedziału rejestru kontrolki na $P $.

W tym momencie możemy to zrobić, ale jest to w jakiś sposób niesatysfakcjonujący, że nasza nowa operacja nie "działa", podobnie jak zastosowanie `Controlled` Funktor.
