...
## <a name="language-features"></a>Funkcje języka
...
Wyrażenie warunkowe jest wyrażeniem `condition ? caseTrue | caseFalse`formularza.
Jak widać w powyższym przykładzie, iteracja przez tablice jest teraz również obsługiwana.

Alokacje w ramach `using` i `borrowing` są nowo możliwe dla pojedynczych tablic qubits, qubit i krotek.
```qsharp
    using (q = Qubit()) {
...
Wprowadzono dwie nowe dyrektywy generatora specjalizacji `invert` i `distribute` oprócz `auto`ów, aby zwiększyć kontrolę nad sposobem generowania specjalizacji Funktor.
Jawna deklaracja domyślnego `body` specjalizacji w ramach operacji nie jest już obowiązkowa.
...
Modyfikatory powiążą się ściśle z innymi wyrażeniami Combinator, a w szczególności większym niż wywołanie Combinator `)``(`. 
`Adjoint`, `Controlled`i nowy operator przyrostka "unotoka" `!` jest traktowany jako modyfikatory wyrażeń. Traktowanie `Adjoint` i `Controlled` jako modyfikatory wyrażeń eliminuje konieczność stosowania nawiasów w wielu przypadkach. Poniższe wyrażenia na przykład są prawidłowe w przypadku `Op`, operacji jednostkowej `opArr` tablicy operacji jednostkowych i `arg` odpowiednim argumentem: 
```qsharp
    Adjoint Op (arg);
    Controlled opArr[i] (arg);
```
Skrypt migracji spowoduje usunięcie większości zbędnych nawiasów i pełne korzystanie z nowych funkcji. Należy jednak pamiętać, że skrypt nie interpretuje kodu i nie będzie uwzględniać różnych interpretacji `Controlled` Funktor w rzadkich przypadkach (dotyczy to tylko w połączeniu z częściową aplikacjami).

Nowy modyfikator `!` rzutowania typu zdefiniowanego przez użytkownika na jego typ podstawowy. To rzutowanie zostało uznane za rzutowane w poprzednich wersjach i zostało wykonane automatycznie. W przypadku nowej interpretacji typów zdefiniowanych przez użytkownika ta wartość nie jest już potrzebna, a rzutowanie musi być jawnie wykonane.    
...
```
Jak widać w przykładzie, `!` wiąże się z modyfikatorami prefiksów, takimi jak `Controlled` i `Adjoint`.

## <a name="BreakingChanges"></a>Istotne zmiany 
...
Poniższy przykład ilustruje sposób użycia funkcji `ToOperation`, aby zmienić `Square` funkcji na `op`operacji: 
```qsharp
...
## <a name="Migration"></a>Migrowanie istniejących projektów do Q # 0,3 
...
2. W edytorze tekstów Edytuj plik. csproj, aby zmienić wersję elementu "Microsoft. Quantum" `PackageReference` na wersję 0.3.1811.2802-Preview, na przykład:
```xml
...
## <a name="MigrationScript"></a>Skrypt migracji 
...
- Projekt został skompilowany przy użyciu poprzedniej wersji zestawu Quantum Development Kit, aby upewnić się, że migracja będzie prawdopodobnie zakończona pomyślnie. Ten krok można pominąć, używając parametru `-Force` do skryptu.
...
> Aby zezwolić na uruchamianie skryptów, takich jak `qdk-migrate.ps1` w systemie Windows, może być konieczne zmiana zasad wykonywania.
