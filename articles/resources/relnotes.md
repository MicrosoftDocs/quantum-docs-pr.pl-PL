...
## <a name="version-0152101126807"></a>0.15.2101.126807 wersja

*Data wydania: 29 stycznia, 2021*

- Dodano szablony projektu do Q# kompilatora dla plików wykonywalnych przeznaczonych dla dostawców IonQ i Honeywell
- Aktualizowanie I Q# wyróżnianie składni jądra w celu uwzględnienia zmian Q# w składni wprowadzonej w wersji [0.15.2101125897](#version-0152101125897)
- Poprawka do obsługi przekazywania tablic jako argumentów wejściowych do Q# programów przesyłanych do usługi Quantum platformy Azure za pośrednictwem `%azure.execute` , zobacz problem [#401](https://github.com/microsoft/iqsharp/issues/401)
- Wystąpił błąd "odmowa uprawnień" przy użyciu `az` wewnątrz `iqsharp-base` obrazów platformy Docker. zobacz problem [#404](https://github.com/microsoft/iqsharp/issues/404)

## <a name="version-0152101125897"></a>0.15.2101125897 wersja

*Data wydania: 26 stycznia 2021*

- Uproszczona alokacja qubit, która zapewnia bardziej wygodną składnię do alokowania qubits, [Zobacz szczegóły w Q# repozytorium języka](https://github.com/microsoft/qsharp-language/blob/main/Approved/1-implicitly-scoped-qubit-allocation.md).
- Utworzono repozytorium QDK-Python, które obejmuje `azure-quantum` klienta języka Python do przesyłania zadań optymalizacji inspirowanej Quantum do usługi Azure Quantum Service, a także, w `qdk` tym `qdk.chemistry` , dla biblioteki chemii opartej na języku Python, Q# która obejmuje wizualizację i funkcje w celu generowania plików wejściowych dla kilku pakietów chemii, takich jak NWChem, Psi4 i OpenMolcas.
- Nawiasy są teraz opcjonalne dla typów operacji i funkcji oraz `if` `elif` instrukcji,, `while` i `until` . Nawiasy dla `for` `use` i `borrow` instrukcje są przestarzałe.
- Ulepszone oszacowania szerokości dla optymalnej głębokości, [Zobacz szczegóły](https://github.com/MicrosoftDocs/quantum-docs-pr/pull/1159).
- Zastosuj operację jednostkową podaną jako jawną macierzą przy użyciu `ApplyUnitary` ([QuantumLibraries # 391](https://github.com/microsoft/QuantumLibraries/pull/391), udział zewnętrzny przez Dmytro Fedoriaka)
- Naprawiono https://github.com/microsoft/iqsharp/issues/387 , ograniczając wpływ na wydajność podczas Q# uruchamiania jądra.

## <a name="version-0142011120240"></a>0.14.2011120240 wersja

*Data wydania: 25 listopada, 2020*

- Ulepszona wydajność kompilatora z powodu szybszego ładowania odwołań.
- Dodano [gramatykę Q# antlr](https://github.com/microsoft/qsharp-language/tree/main/Specifications/Language/5_Grammar) do Q# specyfikacji języka.
- Zaktualizowano [ `Microsoft.Quantum.Preparation` przestrzeń nazw](xref:Microsoft.Quantum.Preparation) , aby była bardziej spójna z przewodnikiem stylu i zasadami projektowania interfejsu API oraz aby zapewnić obsługę oczyszczonych Stanów mieszanych z dodatkowymi danymi (zobacz temat [propozycja](https://github.com/microsoft/QuantumLibraries/issues/344), [uwagi dotyczące przeglądu](https://github.com/microsoft/QuantumLibraries/blob/main/Design/meetings/2020/api-design-2020-11-05.md) i żądań ściągnięcia [#212](https://github.com/microsoft/QuantumLibraries/pull/212), [#322](https://github.com/microsoft/QuantumLibraries/pull/322), [#375](https://github.com/microsoft/QuantumLibraries/pull/375), [#376](https://github.com/microsoft/QuantumLibraries/pull/376)).
- Nawiasy wokół powtórzonych wyrażeń wywołań są teraz opcjonalne: `(Foo(x))(y)` mogą być zapisywane jako `Foo(x)(y)` .
- Użytkownicy rozszerzeń programu Visual Studio lub Visual Studio Code, którzy zainstalowali platformę .NET 5 lub Visual Studio 16,8, mogą zostać poproszeni o zainstalowanie programu .NET Core 3,1, aby kontynuować pracę z rozszerzeniami.
...
Ta wersja zawiera poprawki błędów i funkcje dotyczące problemów raportowanych przez społeczność.
