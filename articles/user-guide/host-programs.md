---
title: 'Sposoby uruchamiania :::no-loc(Q#)::: programu'
description: 'Przegląd różnych sposobów uruchamiania :::no-loc(Q#)::: programów. Z poziomu wiersza polecenia, :::no-loc(Q#)::: notesów Jupyter i klasycznych programów do obsługi hostów w języku Python lub platformie .NET.'
author: gillenhaalb
ms.author: a-gibec
ms.date: 05/15/2020
ms.topic: article
...
---
# <a name="ways-to-run-a-no-locq-program"></a><span data-ttu-id="ee278-104">Sposoby uruchamiania :::no-loc(Q#)::: programu</span><span class="sxs-lookup"><span data-stu-id="ee278-104">Ways to run a :::no-loc(Q#)::: program</span></span>
...
<span data-ttu-id="ee278-106">Oznacza to jednak, że nowi :::no-loc(Q#)::: Użytkownicy mogą odmylić lub przeciążyć wiele opcji dostępnych w [przewodniku instalacji](xref:microsoft.quantum.install).</span><span class="sxs-lookup"><span data-stu-id="ee278-106">However, this also means that new :::no-loc(Q#)::: users may find themselves confused or overwhelmed by the numerous options found in the [install guide](xref:microsoft.quantum.install).</span></span>
<span data-ttu-id="ee278-107">Na tej stronie wyjaśnimy, co :::no-loc(Q#)::: się dzieje, gdy program jest uruchomiony, i Porównaj różne sposoby, w których użytkownicy mogą to robić.</span><span class="sxs-lookup"><span data-stu-id="ee278-107">On this page, we explain what happens when a :::no-loc(Q#)::: program is run, and compare the different ways in which users can do so.</span></span>

<span data-ttu-id="ee278-108">Podstawowa różnica polega na tym, że :::no-loc(Q#)::: można uruchomić:</span><span class="sxs-lookup"><span data-stu-id="ee278-108">A primary distinction is that :::no-loc(Q#)::: can be run:</span></span>
- <span data-ttu-id="ee278-109">jako aplikacja autonomiczna, w której :::no-loc(Q#)::: jest jedynym używanym językiem, a program jest wywoływany bezpośrednio.</span><span class="sxs-lookup"><span data-stu-id="ee278-109">as a standalone application, where :::no-loc(Q#)::: is the only language involved and the program is invoked directly.</span></span> <span data-ttu-id="ee278-110">Dwie metody faktycznie należą do tej kategorii:</span><span class="sxs-lookup"><span data-stu-id="ee278-110">Two methods actually fall in this category:</span></span>
  ...
  - <span data-ttu-id="ee278-112">:::no-loc(Q#)::: Notesy Jupyter</span><span class="sxs-lookup"><span data-stu-id="ee278-112">:::no-loc(Q#)::: Jupyter Notebooks</span></span>
...
<span data-ttu-id="ee278-114">Aby zapoznać się z najlepszymi procesami i ich różnicami, należy wziąć pod uwagę prosty :::no-loc(Q#)::: program i porównać sposoby jego działania.</span><span class="sxs-lookup"><span data-stu-id="ee278-114">To best understand these processes and their differences, we consider a simple :::no-loc(Q#)::: program and compare the ways it can be run.</span></span>

## <a name="basic-no-locq-program"></a><span data-ttu-id="ee278-115">:::no-loc(Q#):::Program Basic</span><span class="sxs-lookup"><span data-stu-id="ee278-115">Basic :::no-loc(Q#)::: program</span></span>
...
<span data-ttu-id="ee278-118">W programie :::no-loc(Q#)::: można to wykonać za pomocą następującego kodu:</span><span class="sxs-lookup"><span data-stu-id="ee278-118">In :::no-loc(Q#):::, this would be performed by the following code:</span></span>
...
<span data-ttu-id="ee278-119">Jednak ten kod nie może być uruchamiany przez :::no-loc(Q#)::: .</span><span class="sxs-lookup"><span data-stu-id="ee278-119">However, this code alone can't be run by :::no-loc(Q#):::.</span></span>
...
<span data-ttu-id="ee278-123">Chociaż przykłady na tej stronie składają się tylko z :::no-loc(Q#)::: *operacji* , wszystkie koncepcje, które będziemy omawiać, odnoszą się również do :::no-loc(Q#)::: *funkcji* , a tym samym odwołują się do nich. *callables*</span><span class="sxs-lookup"><span data-stu-id="ee278-123">While the examples on this page only consist of :::no-loc(Q#)::: *operations* , all of the concepts we will discuss pertain equally to :::no-loc(Q#)::: *functions* , and therefore we refer to them collectively as *callables* .</span></span> <span data-ttu-id="ee278-124">Różnice między nimi są omówione w oparciu o [ :::no-loc(Q#)::: kwestie podstawowe: operacje i funkcje](xref:microsoft.quantum.guide.basics#q-operations-and-functions)oraz więcej szczegółowych informacji na temat ich definiowania można znaleźć w temacie [Operations and Functions](xref:microsoft.quantum.guide.operationsfunctions).</span><span class="sxs-lookup"><span data-stu-id="ee278-124">Their differences are discussed at [:::no-loc(Q#)::: basics: operations and functions](xref:microsoft.quantum.guide.basics#q-operations-and-functions), and more details on defining them can be found at [Operations and functions](xref:microsoft.quantum.guide.operationsfunctions).</span></span>

### <a name="callable-defined-in-a-no-locq-file"></a><span data-ttu-id="ee278-125">Zdefiniowane w :::no-loc(Q#)::: pliku</span><span class="sxs-lookup"><span data-stu-id="ee278-125">Callable defined in a :::no-loc(Q#)::: file</span></span>

<span data-ttu-id="ee278-126">Wywołanie jest precyzyjnie wywoływane i uruchamiane przez :::no-loc(Q#)::: .</span><span class="sxs-lookup"><span data-stu-id="ee278-126">The callable is precisely what's called and run by :::no-loc(Q#):::.</span></span>
<span data-ttu-id="ee278-127">Jednak wymaga to kilku dodatkowych uzupełnień w celu przeprowadzenia pełnego `*.qs` :::no-loc(Q#)::: pliku.</span><span class="sxs-lookup"><span data-stu-id="ee278-127">However, it requires a few more additions to comprise a full `*.qs` :::no-loc(Q#)::: file.</span></span>

<span data-ttu-id="ee278-128">Wszystkie :::no-loc(Q#)::: typy i elementy, które można ustalić (zarówno te, które zostały zdefiniowane, jak i wewnętrzne dla języka) są zdefiniowane w *przestrzeni nazw* , co zapewnia pełną nazwę, do której można się odwoływać.</span><span class="sxs-lookup"><span data-stu-id="ee278-128">All :::no-loc(Q#)::: types and callables (both those you define and those intrinsic to the language) are defined within *namespaces* , which provide each a full name that can then be referenced.</span></span>

<span data-ttu-id="ee278-129">Na przykład [`H`](xref:Microsoft.Quantum.Intrinsic.H) [`MResetZ`](xref:Microsoft.Quantum.Measurement.MResetZ) operacje i są dostępne w [`Microsoft.Quantum.Instrinsic`](xref:Microsoft.Quantum.Intrinsic) [`Microsoft.Quantum.Measurement`](xref:Microsoft.Quantum.Measurement) przestrzeniach nazw i (części [ :::no-loc(Q#)::: bibliotek standardowych](xref:microsoft.quantum.qsharplibintro)).</span><span class="sxs-lookup"><span data-stu-id="ee278-129">For example, the [`H`](xref:Microsoft.Quantum.Intrinsic.H) and [`MResetZ`](xref:Microsoft.Quantum.Measurement.MResetZ) operations are found in the [`Microsoft.Quantum.Instrinsic`](xref:Microsoft.Quantum.Intrinsic) and [`Microsoft.Quantum.Measurement`](xref:Microsoft.Quantum.Measurement) namespaces (part of the [:::no-loc(Q#)::: Standard Libraries](xref:microsoft.quantum.qsharplibintro)).</span></span>
...
<span data-ttu-id="ee278-132">W związku z tym pełen :::no-loc(Q#)::: plik zawierający naszą operację będzie obejmował Definiowanie własnej przestrzeni nazw, otwierając obszary nazw dla tych, których używa nasze operacje, a następnie naszej operacji:</span><span class="sxs-lookup"><span data-stu-id="ee278-132">The full :::no-loc(Q#)::: file containing our operation would therefore consist of defining our own namespace, opening the namespaces for those callables our operation uses, and then our operation:</span></span>
...
<span data-ttu-id="ee278-138">Teraz ogólny model przebiegu :::no-loc(Q#)::: programu zostanie wyczyszczony.</span><span class="sxs-lookup"><span data-stu-id="ee278-138">Now the general run model of a :::no-loc(Q#)::: program becomes clear.</span></span>
...
<span data-ttu-id="ee278-140">Uzyskują również dostęp do tych elementów z dowolnej [ :::no-loc(Q#)::: biblioteki](xref:microsoft.quantum.libraries), ale muszą one być przywoływane za pośrednictwem ich pełnej nazwy albo za pomocą `open` instrukcji opisanych powyżej.</span><span class="sxs-lookup"><span data-stu-id="ee278-140">It also access those from any of the [:::no-loc(Q#)::: libraries](xref:microsoft.quantum.libraries), but those must be referenced either via their full name, or through the use of `open` statements described above.</span></span>
...
<span data-ttu-id="ee278-144">Do tej pory opisano, co się dzieje, gdy :::no-loc(Q#)::: zostanie uruchomione określone wywoływanie.</span><span class="sxs-lookup"><span data-stu-id="ee278-144">So far, we've described what happens when a specific :::no-loc(Q#)::: callable is being run.</span></span>
<span data-ttu-id="ee278-145">Bez względu na to, czy :::no-loc(Q#)::: jest używany w aplikacji autonomicznej, czy za pomocą programu hosta, ten ogólny proces jest większy lub mniejszy---tym samym elastyczność QDK.</span><span class="sxs-lookup"><span data-stu-id="ee278-145">Regardless of whether :::no-loc(Q#)::: is used in a standalone application or with a host program, this general process is more or less the same---hence the QDK's flexibility.</span></span>
<span data-ttu-id="ee278-146">Różnice między sposobami wywołania zestawu Quantum Development Kit w ten sposób ujawniają się w *sposób* , w jaki :::no-loc(Q#)::: wywołania wywoływane do uruchomienia i w jaki sposób są zwracane wszystkie wyniki.</span><span class="sxs-lookup"><span data-stu-id="ee278-146">The differences between the ways of calling into the Quantum Development Kit therefore reveal themselves in *how* that :::no-loc(Q#)::: callable is called to be run, and in what manner any results are returned.</span></span>
...
- <span data-ttu-id="ee278-148">Wskazuje, który :::no-loc(Q#)::: możliwy do uruchomienia ma być uruchamiany</span><span class="sxs-lookup"><span data-stu-id="ee278-148">Indicating which :::no-loc(Q#)::: callable is to be run</span></span>
...
<span data-ttu-id="ee278-152">Najpierw omówiono, jak to zrobić za pomocą :::no-loc(Q#)::: aplikacji autonomicznej z wiersza polecenia, a następnie kontynuować korzystanie z programów obsługi języka Python i C#.</span><span class="sxs-lookup"><span data-stu-id="ee278-152">First, we discuss how this is done with the :::no-loc(Q#)::: standalone application from the command prompt, and then proceed to using Python and C# host programs.</span></span>
<span data-ttu-id="ee278-153">Firma Microsoft rezerwuje autonomiczną aplikację :::no-loc(Q#)::: dla notesów Jupyter w ciągu ostatnich, ponieważ w przeciwieństwie do pierwszych trzech funkcja podstawowa nie Wyśrodkowuje :::no-loc(Q#)::: plików lokalnych.</span><span class="sxs-lookup"><span data-stu-id="ee278-153">We reserve the standalone application of :::no-loc(Q#)::: Jupyter Notebooks for last, because unlike the first three, it's primary functionality does not center around a local :::no-loc(Q#)::: file.</span></span>

> [!NOTE]
> <span data-ttu-id="ee278-154">Chociaż nie jest to zilustrowane w tych przykładach, jedna ze współdziałania między metodami Run polega na tym, że wszystkie komunikaty, które są drukowane z wewnątrz :::no-loc(Q#)::: programu ( [`Message`](xref:Microsoft.Quantum.Intrinsic.Message) [`DumpMachine`](xref:Microsoft.Quantum.Diagnostics.DumpMachine) na przykład), zazwyczaj będą zawsze drukowane do odpowiedniej konsoli.</span><span class="sxs-lookup"><span data-stu-id="ee278-154">Although we don't illustrate it in these examples, one commonality between the run methods is that any messages printed from inside the :::no-loc(Q#)::: program (by way of [`Message`](xref:Microsoft.Quantum.Intrinsic.Message) or [`DumpMachine`](xref:Microsoft.Quantum.Diagnostics.DumpMachine), for example) will typically always be printed to the respective console.</span></span>

## <a name="no-locq-from-the-command-prompt"></a><span data-ttu-id="ee278-155">:::no-loc(Q#)::: z wiersza polecenia</span><span class="sxs-lookup"><span data-stu-id="ee278-155">:::no-loc(Q#)::: from the command prompt</span></span>
<span data-ttu-id="ee278-156">Jednym z najprostszych sposobów na rozpoczęcie pisania :::no-loc(Q#)::: programów jest uniknięcie przejmowania informacji o osobnych plikach i drugim języku.</span><span class="sxs-lookup"><span data-stu-id="ee278-156">One of the easiest ways to get started writing :::no-loc(Q#)::: programs is to avoid worrying about separate files and a second language altogether.</span></span>
<span data-ttu-id="ee278-157">Używanie Visual Studio Code lub Visual Studio z rozszerzeniem QDK umożliwia bezproblemowe przepływ pracy, w którym uruchamiamy wywoływany :::no-loc(Q#)::: z tylko jednego :::no-loc(Q#)::: pliku.</span><span class="sxs-lookup"><span data-stu-id="ee278-157">Using Visual Studio Code or Visual Studio with the QDK extension allows for a seamless work flow in which we run :::no-loc(Q#)::: callables from only a single :::no-loc(Q#)::: file.</span></span>
...
<span data-ttu-id="ee278-160">Najprostszym przepływem pracy jest to, że lokalizacja katalogu terminalu jest taka sama jak w przypadku :::no-loc(Q#)::: pliku, który można łatwo obsługiwać podczas :::no-loc(Q#)::: edycji plików przy użyciu zintegrowanego terminalu w vs Code, na przykład.</span><span class="sxs-lookup"><span data-stu-id="ee278-160">The simplest workflow is when the terminal's directory location is the same as the :::no-loc(Q#)::: file, which can be easily handled alongside :::no-loc(Q#)::: file editing by using the integrated terminal in VS Code, for example.</span></span>
<span data-ttu-id="ee278-161">Jednak [ `dotnet run` polecenie](https://docs.microsoft.com/dotnet/core/tools/dotnet-run) akceptuje wiele opcji, a program może być również uruchamiany z innej lokalizacji, po prostu dostarczając `--project <PATH>` lokalizację :::no-loc(Q#)::: pliku.</span><span class="sxs-lookup"><span data-stu-id="ee278-161">However, the [`dotnet run` command](https://docs.microsoft.com/dotnet/core/tools/dotnet-run) accepts numerous options, and the program can also be run from a different location by simply providing `--project <PATH>` with the location of the :::no-loc(Q#)::: file.</span></span>


### <a name="add-entry-point-to-no-locq-file"></a><span data-ttu-id="ee278-162">Dodaj punkt wejścia do :::no-loc(Q#)::: pliku</span><span class="sxs-lookup"><span data-stu-id="ee278-162">Add entry point to :::no-loc(Q#)::: file</span></span>

<span data-ttu-id="ee278-163">Większość :::no-loc(Q#)::: plików będzie zawierać więcej niż jeden możliwy do oddzwonienia, więc w naturalny sposób musimy poinformować kompilator o *tym, który* jest możliwy do uruchomienia, gdy udostępnimy `dotnet run` polecenie.</span><span class="sxs-lookup"><span data-stu-id="ee278-163">Most :::no-loc(Q#)::: files will contain more than one callable, so naturally we need to let the compiler know *which* callable to run when we provide the `dotnet run` command.</span></span>
<span data-ttu-id="ee278-164">Jest to wykonywane z prostą zmianą dla :::no-loc(Q#)::: samego pliku:</span><span class="sxs-lookup"><span data-stu-id="ee278-164">This is done with a simple change to the :::no-loc(Q#)::: file itself:</span></span> 
...
### <a name="non-no-locq-dotnet-run-options"></a><span data-ttu-id="ee278-193">Opcje inne niż :::no-loc(Q#)::: `dotnet run`</span><span class="sxs-lookup"><span data-stu-id="ee278-193">Non-:::no-loc(Q#)::: `dotnet run` options</span></span>

<span data-ttu-id="ee278-194">Jak opisano powyżej przy użyciu `--project` opcji, [ `dotnet run` polecenie](https://docs.microsoft.com/dotnet/core/tools/dotnet-run) akceptuje również opcje niezwiązane z wywoływanymi :::no-loc(Q#)::: argumentami.</span><span class="sxs-lookup"><span data-stu-id="ee278-194">As we briefly mentioned above with the `--project` option, the [`dotnet run` command](https://docs.microsoft.com/dotnet/core/tools/dotnet-run) also accepts options unrelated to the :::no-loc(Q#)::: callable arguments.</span></span>
<span data-ttu-id="ee278-195">W przypadku udostępniania obu rodzajów opcji `dotnet` należy najpierw podać opcje specyficzne, po których następuje ogranicznik `--` , a następnie :::no-loc(Q#)::: Opcje specyficzne dla programu.</span><span class="sxs-lookup"><span data-stu-id="ee278-195">If providing both kinds of options, the `dotnet`-specific options must be provided first, followed by a delimeter `--`, and then the :::no-loc(Q#):::-specific options.</span></span>
...
<span data-ttu-id="ee278-201">W Nutshell sytuacja zawiera teraz plik programu hosta (na przykład `*.py` lub `*.cs` ) w tej samej lokalizacji, w której znajduje się :::no-loc(Q#)::: plik.</span><span class="sxs-lookup"><span data-stu-id="ee278-201">In a nutshell, the situation now includes a host program file (for example, `*.py` or `*.cs`) in the same location as our :::no-loc(Q#)::: file.</span></span>
<span data-ttu-id="ee278-202">Teraz jest to program *hosta* , który jest uruchamiany, a gdy jest uruchomiony, może wywołać określone :::no-loc(Q#)::: operacje i funkcje z :::no-loc(Q#)::: pliku.</span><span class="sxs-lookup"><span data-stu-id="ee278-202">It's now the *host* program that gets run, and while it is running, it can call specific :::no-loc(Q#)::: operations and functions from the :::no-loc(Q#)::: file.</span></span>
<span data-ttu-id="ee278-203">Rdzeń współdziałania jest oparty na kompilatorze, dzięki czemu :::no-loc(Q#)::: zawartość pliku jest :::no-loc(Q#)::: dostępna dla programu hosta, dzięki czemu mogą one być wywoływane.</span><span class="sxs-lookup"><span data-stu-id="ee278-203">The core of the interoperability is based on the :::no-loc(Q#)::: compiler making the contents of the :::no-loc(Q#)::: file accessible to the host program so that they can be called.</span></span>

<span data-ttu-id="ee278-204">Jedną z głównych zalet korzystania z programu hosta jest to, że klasyczne dane zwrócone przez :::no-loc(Q#)::: program można następnie przetworzyć w języku hosta.</span><span class="sxs-lookup"><span data-stu-id="ee278-204">One of the main benefits of using a host program is that the classical data returned by the :::no-loc(Q#)::: program can then be further processed in the host language.</span></span>
<span data-ttu-id="ee278-205">Może to obejmować pewne zaawansowane przetwarzanie danych (np. coś, które nie może być wykonane wewnętrznie w programie :::no-loc(Q#)::: ), a następnie wywoływanie dalszych :::no-loc(Q#)::: akcji na podstawie tych wyników, jak również do wykreślania :::no-loc(Q#)::: wyników.</span><span class="sxs-lookup"><span data-stu-id="ee278-205">This could consist of some advanced data processing (for example, something that can't be performed internally in :::no-loc(Q#):::), and then calling further :::no-loc(Q#)::: actions based on those results, or something as simple as plotting the :::no-loc(Q#)::: results.</span></span>
...
> [!NOTE]
> <span data-ttu-id="ee278-208">`@EntryPoint()`Atrybut używany dla :::no-loc(Q#)::: aplikacji nie może być używany z programami hosta.</span><span class="sxs-lookup"><span data-stu-id="ee278-208">The `@EntryPoint()` attribute used for :::no-loc(Q#)::: applications cannot be used with host programs.</span></span>
> <span data-ttu-id="ee278-209">Jeśli jest obecny w :::no-loc(Q#)::: pliku wywoływanym przez hosta, zostanie zgłoszony błąd.</span><span class="sxs-lookup"><span data-stu-id="ee278-209">An error will be raised if it is present in the :::no-loc(Q#)::: file being called by a host.</span></span> 

<span data-ttu-id="ee278-210">Do pracy z różnymi programami hosta nie są wymagane żadne zmiany do `*.qs` :::no-loc(Q#)::: pliku.</span><span class="sxs-lookup"><span data-stu-id="ee278-210">To work with different host programs, there are no changes required to a `*.qs` :::no-loc(Q#)::: file.</span></span>
<span data-ttu-id="ee278-211">Następujące implementacje programu hosta działają z tym samym :::no-loc(Q#)::: plikiem:</span><span class="sxs-lookup"><span data-stu-id="ee278-211">The following host program implementations all work with the same :::no-loc(Q#)::: file:</span></span>
...
1. <span data-ttu-id="ee278-215">Zaimportuj `qsharp` moduł, który rejestruje moduł ładujący modułu pod kątem :::no-loc(Q#)::: współdziałania.</span><span class="sxs-lookup"><span data-stu-id="ee278-215">Import the `qsharp` module, which registers the module loader for :::no-loc(Q#)::: interoperability.</span></span> 
    <span data-ttu-id="ee278-216">Dzięki temu :::no-loc(Q#)::: przestrzenie nazw mogą być wyświetlane jako moduły języka Python, z których można "zaimportować" :::no-loc(Q#)::: .</span><span class="sxs-lookup"><span data-stu-id="ee278-216">This allows :::no-loc(Q#)::: namespaces to appear as Python modules, from which we can "import" :::no-loc(Q#)::: callables.</span></span>
    <span data-ttu-id="ee278-217">Należy zauważyć, że nie jest to technicznie możliwe do :::no-loc(Q#)::: zaimportowania, ale raczej elementy zastępcze języka Python, które umożliwiają wywoływanie.</span><span class="sxs-lookup"><span data-stu-id="ee278-217">Note that it is technically not the :::no-loc(Q#)::: callables themselves which are imported, but rather Python stubs which allow calling into them.</span></span>
...
2. <span data-ttu-id="ee278-220">Zaimportuj te :::no-loc(Q#)::: , które zostały wywołane bezpośrednio---w tym przypadku, `MeasureSuperposition` i `MeasureSuperpositionArray` .</span><span class="sxs-lookup"><span data-stu-id="ee278-220">Import those :::no-loc(Q#)::: callables which we will directly invoke---in this case, `MeasureSuperposition` and `MeasureSuperpositionArray`.</span></span>
    ```python
    import qsharp
    from NamespaceName import MeasureSuperposition, MeasureSuperpositionArray
    ```
    <span data-ttu-id="ee278-221">Po `qsharp` zaimportowaniu modułu można także zaimportować możliwe do odwoływać się bezpośrednio z :::no-loc(Q#)::: przestrzeni nazw biblioteki.</span><span class="sxs-lookup"><span data-stu-id="ee278-221">With the `qsharp` module imported, you can also import callables directly from the :::no-loc(Q#)::: library namespaces.</span></span>
...
<span data-ttu-id="ee278-238">Aby załadować pakiety zewnętrzne zawierające :::no-loc(Q#)::: kod, użyj [ `qsharp.packages` interfejsu API](https://docs.microsoft.com/python/qsharp-core/qsharp.packages.packages).</span><span class="sxs-lookup"><span data-stu-id="ee278-238">To load external packages containing :::no-loc(Q#)::: code, use the [`qsharp.packages` API](https://docs.microsoft.com/python/qsharp-core/qsharp.packages.packages).</span></span>

<span data-ttu-id="ee278-239">Jeśli :::no-loc(Q#)::: kod w bieżącym folderze zależy od zewnętrznych projektów lub pakietów, mogą pojawić `import qsharp` się błędy, ponieważ zależności nie zostały jeszcze załadowane.</span><span class="sxs-lookup"><span data-stu-id="ee278-239">If the :::no-loc(Q#)::: code in the current folder depends on external projects or packages, you may see errors when running `import qsharp`, since the dependencies have not yet been loaded.</span></span>
<span data-ttu-id="ee278-240">Aby załadować wymagane pakiety zewnętrzne lub :::no-loc(Q#)::: projekty podczas wykonywania `import qsharp` polecenia, upewnij się, że folder z skryptem języka Python zawiera `.csproj` plik, którego dotyczy odwołanie `Microsoft.Quantum.Sdk` .</span><span class="sxs-lookup"><span data-stu-id="ee278-240">To load required external packages or :::no-loc(Q#)::: projects during the `import qsharp` command, ensure that the folder with the Python script contains a `.csproj` file which references `Microsoft.Quantum.Sdk`.</span></span> <span data-ttu-id="ee278-241">W tym `.csproj` celu należy dodać właściwość `<IQSharpLoadAutomatically>true</IQSharpLoadAutomatically>` do `<PropertyGroup>` .</span><span class="sxs-lookup"><span data-stu-id="ee278-241">In that `.csproj`, add the property `<IQSharpLoadAutomatically>true</IQSharpLoadAutomatically>` to the `<PropertyGroup>`.</span></span> <span data-ttu-id="ee278-242">Spowoduje to wymuszenie :::no-loc(Q#)::: cyklicznego ładowania dowolnego `ProjectReference` lub `PackageReference` elementów znalezionych w tym `.csproj` czasie w `import qsharp` poleceniu.</span><span class="sxs-lookup"><span data-stu-id="ee278-242">This will instruct I:::no-loc(Q#)::: to recursively load any `ProjectReference` or `PackageReference` items found in that `.csproj` during the `import qsharp` command.</span></span>

<span data-ttu-id="ee278-243">Na przykład poniżej przedstawiono prosty `.csproj` plik, który powoduje :::no-loc(Q#)::: automatyczne załadowanie `Microsoft.Quantum.Chemistry` pakietu:</span><span class="sxs-lookup"><span data-stu-id="ee278-243">For example, here is a simple `.csproj` file that causes I:::no-loc(Q#)::: to automatically load the `Microsoft.Quantum.Chemistry` package:</span></span>
...
<span data-ttu-id="ee278-249">:::no-loc(Q#):::Kompilator działa w tym miejscu przez wygenerowanie równoważnej nazwy przestrzeni nazw C# z :::no-loc(Q#)::: przestrzeni nazw w :::no-loc(Q#)::: pliku.</span><span class="sxs-lookup"><span data-stu-id="ee278-249">The :::no-loc(Q#)::: compiler works here by generating an equivalently named C# namespace from the :::no-loc(Q#)::: namespace in our :::no-loc(Q#)::: file.</span></span>
<span data-ttu-id="ee278-250">Następnie generuje on równoważną nazwę klasy C# dla każdego obiektu :::no-loc(Q#)::: lub zdefiniowanego w nim typu.</span><span class="sxs-lookup"><span data-stu-id="ee278-250">It further generates an equivalently named C# class for each of the :::no-loc(Q#)::: callables or types defined therein.</span></span>

<span data-ttu-id="ee278-251">Najpierw udostępniamy wszelkie klasy używane w naszym programie hosta z `using` instrukcjami, które są w przybliżeniu analagous do `open` instrukcji w naszym :::no-loc(Q#)::: pliku:</span><span class="sxs-lookup"><span data-stu-id="ee278-251">First, we make any classes used in our host program available with `using` statements, which are roughly analagous to the `open` statements in our :::no-loc(Q#)::: file:</span></span>
...
<span data-ttu-id="ee278-255">Po powrocie do programu :::no-loc(Q#)::: należy utworzyć wystąpienie dowolnego komputera docelowego, na którym zostaną uruchomione nasze operacje.</span><span class="sxs-lookup"><span data-stu-id="ee278-255">Getting back to :::no-loc(Q#):::, we must create an instance of whatever target machine we will run our operations on.</span></span>
...
<span data-ttu-id="ee278-258">Każda Klasa języka C# wygenerowana na podstawie :::no-loc(Q#)::: operacji ma `Run` metodę, a pierwszy argument, który musi być wystąpieniem maszyny docelowej.</span><span class="sxs-lookup"><span data-stu-id="ee278-258">Each C# class generated from the :::no-loc(Q#)::: operations have a `Run` method, the first argument of which must be the target machine instance.</span></span>
...
<span data-ttu-id="ee278-262">Jeśli możliwy do odrzucania :::no-loc(Q#)::: nie ma żadnych zwracanych (na przykład zwraca typ `Unit` ), przebieg można nadal wykonać w taki sam sposób, bez przypisywania go do zmiennej.</span><span class="sxs-lookup"><span data-stu-id="ee278-262">If the :::no-loc(Q#)::: callable does not have any returns (for example, it has return type `Unit`), then the run can still be done in the same manner without assigning it to a variable.</span></span>
...
<span data-ttu-id="ee278-265">Wszystkie argumenty do wywoływanych :::no-loc(Q#)::: są po prostu przekazane jako dodatkowe argumenty po maszynie docelowej.</span><span class="sxs-lookup"><span data-stu-id="ee278-265">Any arguments to the :::no-loc(Q#)::: callable are simply passed as additional arguments after the target machine.</span></span>
...
> [!NOTE]
> <span data-ttu-id="ee278-270">Ze względu na współdziałanie kompilatora z przestrzeniami nazw możemy Alternatywnie wykorzystać nasze możliwe do przedzwonienia :::no-loc(Q#)::: bez `using NamespaceName;` instrukcji i po prostu dopasowując do niej tytuł przestrzeni nazw języka C#.</span><span class="sxs-lookup"><span data-stu-id="ee278-270">Due to the compiler's interoperability with namespaces, we could alternatively make our :::no-loc(Q#)::: callables available without the `using NamespaceName;` statement, and simply matching the C# namespace title to it.</span></span>
...
## <a name="no-locq-jupyter-notebooks"></a><span data-ttu-id="ee278-281">:::no-loc(Q#)::: Notesy Jupyter</span><span class="sxs-lookup"><span data-stu-id="ee278-281">:::no-loc(Q#)::: Jupyter Notebooks</span></span>
<span data-ttu-id="ee278-282">:::no-loc(Q#)::: Notesy Jupyter używają :::no-loc(Q#)::: jądra I, które umożliwia definiowanie, kompilowanie i uruchamianie wywoływanych elementów :::no-loc(Q#)::: w pojedynczym notesie---wszystkie instrukcje, notatki i inne treści.</span><span class="sxs-lookup"><span data-stu-id="ee278-282">:::no-loc(Q#)::: Jupyter Notebooks make use of the I:::no-loc(Q#)::: kernel, which allows you to define, compile, and run :::no-loc(Q#)::: callables in a single notebook---all alongside instructions, notes, and other content.</span></span>
<span data-ttu-id="ee278-283">Oznacza to, że chociaż istnieje możliwość zaimportowania i użycia zawartości `*.qs` :::no-loc(Q#)::: plików, nie jest to konieczne w modelu uruchomieniowym.</span><span class="sxs-lookup"><span data-stu-id="ee278-283">This means that while it is possible to import and use the contents of `*.qs` :::no-loc(Q#)::: files, they are not necessary in the run model.</span></span>

<span data-ttu-id="ee278-284">Tutaj szczegółowo opisano sposób uruchamiania :::no-loc(Q#)::: operacji zdefiniowanych powyżej, ale bardziej rozległe wprowadzenie do korzystania z :::no-loc(Q#)::: notesów Jupyter jest dostępne na stronie [wprowadzenie do :::no-loc(Q#)::: i Jupyter notesów](https://github.com/microsoft/Quantum/blob/main/samples/getting-started/intro-to-iqsharp/Notebook.ipynb).</span><span class="sxs-lookup"><span data-stu-id="ee278-284">Here, we will detail how to run the :::no-loc(Q#)::: operations defined above, but a more broad introduction to using :::no-loc(Q#)::: Jupyter Notebooks is provided at [Intro to :::no-loc(Q#)::: and Jupyter Notebooks](https://github.com/microsoft/Quantum/blob/main/samples/getting-started/intro-to-iqsharp/Notebook.ipynb).</span></span>
...
<span data-ttu-id="ee278-286">W :::no-loc(Q#)::: Jupyter Notebook wprowadzasz kod w :::no-loc(Q#)::: taki sam sposób jak w przestrzeni nazw :::no-loc(Q#)::: pliku.</span><span class="sxs-lookup"><span data-stu-id="ee278-286">In a :::no-loc(Q#)::: Jupyter Notebook, you enter :::no-loc(Q#)::: code just as we would from inside the namespace of a :::no-loc(Q#)::: file.</span></span>

<span data-ttu-id="ee278-287">Dzięki temu możemy umożliwić dostęp do wywoływanych [ :::no-loc(Q#)::: bibliotek standardowych](xref:microsoft.quantum.qsharplibintro) za pomocą `open` instrukcji dla odpowiednich przestrzeni nazw.</span><span class="sxs-lookup"><span data-stu-id="ee278-287">So, we can enable access to callables from the [:::no-loc(Q#)::: standard libraries](xref:microsoft.quantum.qsharplibintro) with `open` statements for their respective namespaces.</span></span>
...
> [!NOTE]
> <span data-ttu-id="ee278-289">Elementy [Microsoft. Quantum.](xref:Microsoft.Quantum.Intrinsic) Inin i [Microsoft. Quantum. Canon](xref:Microsoft.Quantum.Canon) (na przykład [`H`](xref:Microsoft.Quantum.Intrinsic.H) i [`ApplyToEach`](xref:Microsoft.Quantum.Canon.ApplyToEach) ) są automatycznie dostępne dla operacji zdefiniowanych w komórkach w :::no-loc(Q#)::: notesach Jupyter.</span><span class="sxs-lookup"><span data-stu-id="ee278-289">Callables from [Microsoft.Quantum.Intrinsic](xref:Microsoft.Quantum.Intrinsic) and [Microsoft.Quantum.Canon](xref:Microsoft.Quantum.Canon) (for example, [`H`](xref:Microsoft.Quantum.Intrinsic.H) and [`ApplyToEach`](xref:Microsoft.Quantum.Canon.ApplyToEach)) are automatically available to operations defined within cells in :::no-loc(Q#)::: Jupyter Notebooks.</span></span>
> <span data-ttu-id="ee278-290">Nie jest to jednak prawdziwe w przypadku kodu pochodzącego z zewnętrznych :::no-loc(Q#)::: plików źródłowych (proces pokazywany na początku [ :::no-loc(Q#)::: i w notesach Jupyter](https://github.com/microsoft/Quantum/blob/main/samples/getting-started/intro-to-iqsharp/Notebook.ipynb)).</span><span class="sxs-lookup"><span data-stu-id="ee278-290">However, this is not true for code brought in from external :::no-loc(Q#)::: source files (a process shown at [Intro to :::no-loc(Q#)::: and Jupyter Notebooks](https://github.com/microsoft/Quantum/blob/main/samples/getting-started/intro-to-iqsharp/Notebook.ipynb)).</span></span> 
> 

<span data-ttu-id="ee278-291">Podobnie Definiowanie operacji wymaga tylko pisania :::no-loc(Q#)::: kodu i uruchomienia komórki.</span><span class="sxs-lookup"><span data-stu-id="ee278-291">Similarly, defining operations requires only writing the :::no-loc(Q#)::: code and running the cell.</span></span>
...
<span data-ttu-id="ee278-294">Funkcje do uruchamiania operacji na konkretnych komputerach docelowych są udostępniane za pomocą [ :::no-loc(Q#)::: poleceń Magic](xref:microsoft.quantum.guide.quickref.iqsharp).</span><span class="sxs-lookup"><span data-stu-id="ee278-294">The functionality to run operations on specific target machines is provided via [I:::no-loc(Q#)::: Magic Commands](xref:microsoft.quantum.guide.quickref.iqsharp).</span></span>
...
<span data-ttu-id="ee278-297">Aby przekazać dane wejściowe do :::no-loc(Q#)::: operacji, argumenty mogą być przekazywane jako `key=value` pary do polecenia Run Magic.</span><span class="sxs-lookup"><span data-stu-id="ee278-297">To pass inputs to the :::no-loc(Q#)::: operations, the arguments can be passed as `key=value` pairs to the run magic command.</span></span>
...
### <a name="using-no-locq-code-from-other-projects-or-packages"></a><span data-ttu-id="ee278-300">Używanie :::no-loc(Q#)::: kodu z innych projektów lub pakietów</span><span class="sxs-lookup"><span data-stu-id="ee278-300">Using :::no-loc(Q#)::: code from other projects or packages</span></span>

<span data-ttu-id="ee278-301">Domyślnie :::no-loc(Q#)::: Jupyter Notebook ładuje wszystkie `.qs` pliki w bieżącym folderze i wprowadza ich :::no-loc(Q#)::: operacje i funkcje dostępne do użycia w notesie.</span><span class="sxs-lookup"><span data-stu-id="ee278-301">By default, a :::no-loc(Q#)::: Jupyter Notebook loads all of the `.qs` files in the current folder and makes their :::no-loc(Q#)::: operations and functions available for use from inside the notebook.</span></span> <span data-ttu-id="ee278-302">[ `%who` Magic polecenie](xref:microsoft.quantum.iqsharp.magic-ref.who) wyświetla listę wszystkich aktualnie dostępnych :::no-loc(Q#)::: operacji i funkcji.</span><span class="sxs-lookup"><span data-stu-id="ee278-302">The [`%who` magic command](xref:microsoft.quantum.iqsharp.magic-ref.who) lists all currently-available :::no-loc(Q#)::: operations and functions.</span></span>

<span data-ttu-id="ee278-303">Aby załadować :::no-loc(Q#)::: kod z innego folderu, można użyć [ `%project` polecenia Magic](xref:microsoft.quantum.iqsharp.magic-ref.project) w celu dodania odwołania do `.csproj` pliku dla :::no-loc(Q#)::: projektu (czyli projektu, do którego odwołuje się odwołanie `Microsoft.Quantum.Sdk` ).</span><span class="sxs-lookup"><span data-stu-id="ee278-303">To load :::no-loc(Q#)::: code from another folder, the [`%project` magic command](xref:microsoft.quantum.iqsharp.magic-ref.project) can be used to add a reference to a `.csproj` file for a :::no-loc(Q#)::: project (that is, a project that references `Microsoft.Quantum.Sdk`).</span></span> <span data-ttu-id="ee278-304">To polecenie spowoduje skompilowanie wszystkich `.qs` plików w folderze zawierającym `.csproj` (i podfoldery).</span><span class="sxs-lookup"><span data-stu-id="ee278-304">This command will compile any `.qs` files in the folder containing the `.csproj` (and subfolders).</span></span> <span data-ttu-id="ee278-305">Spowoduje również cykliczne załadowanie wszelkich pakietów, do których odwołuje się `PackageReference` lub :::no-loc(Q#)::: projekty, do których odwołuje się `ProjectReference` ten `.csproj` plik.</span><span class="sxs-lookup"><span data-stu-id="ee278-305">It will also recursively load any packages referenced via `PackageReference` or :::no-loc(Q#)::: projects referenced via `ProjectReference` in that `.csproj` file.</span></span> 

<span data-ttu-id="ee278-306">Przykładowo następujące komórki symulują :::no-loc(Q#)::: operację z zewnętrznego projektu, gdzie Ścieżka projektu jest przywoływana względem bieżącego folderu:</span><span class="sxs-lookup"><span data-stu-id="ee278-306">As an example, the following cells simulate a :::no-loc(Q#)::: operation from an external project, where the project path is referenced relative to the current folder:</span></span>
...
<span data-ttu-id="ee278-307">Aby załadować pakiety zewnętrzne zawierające :::no-loc(Q#)::: kod, użyj [ `%package` polecenia Magic](xref:microsoft.quantum.iqsharp.magic-ref.package).</span><span class="sxs-lookup"><span data-stu-id="ee278-307">To load external packages containing :::no-loc(Q#)::: code, use the [`%package` magic command](xref:microsoft.quantum.iqsharp.magic-ref.package).</span></span>
...
<span data-ttu-id="ee278-309">Aby załadować pakiety zewnętrzne lub :::no-loc(Q#)::: projekty w programie Notes czasu dostarczenie, upewnij się, że folder Notes zawiera `.csproj` plik, którego dotyczy odwołanie `Microsoft.Quantum.Sdk` .</span><span class="sxs-lookup"><span data-stu-id="ee278-309">To load external packages or :::no-loc(Q#)::: projects at notebook intialization time, ensure that the notebook folder contains a `.csproj` file which references `Microsoft.Quantum.Sdk`.</span></span> <span data-ttu-id="ee278-310">W tym `.csproj` celu należy dodać właściwość `<IQSharpLoadAutomatically>true</IQSharpLoadAutomatically>` do `<PropertyGroup>` .</span><span class="sxs-lookup"><span data-stu-id="ee278-310">In that `.csproj`, add the property `<IQSharpLoadAutomatically>true</IQSharpLoadAutomatically>` to the `<PropertyGroup>`.</span></span> <span data-ttu-id="ee278-311">Spowoduje to :::no-loc(Q#)::: załadowanie rekursywnie dowolnego `ProjectReference` lub `PackageReference` elementów znalezionych w tym `.csproj` czasie podczas ładowania notesu.</span><span class="sxs-lookup"><span data-stu-id="ee278-311">This will instruct I:::no-loc(Q#)::: to recursively load any `ProjectReference` or `PackageReference` items found in that `.csproj` at notebook load time.</span></span>

<span data-ttu-id="ee278-312">Na przykład poniżej przedstawiono prosty `.csproj` plik, który powoduje :::no-loc(Q#)::: automatyczne załadowanie `Microsoft.Quantum.Chemistry` pakietu:</span><span class="sxs-lookup"><span data-stu-id="ee278-312">For example, here is a simple `.csproj` file that causes I:::no-loc(Q#)::: to automatically load the `Microsoft.Quantum.Chemistry` package:</span></span>
...
> [!NOTE]
> <span data-ttu-id="ee278-313">Obecnie ta właściwość niestandardowa `<IQSharpLoadAutomatically>` jest wymagana przez :::no-loc(Q#)::: Jupyter Notebook hosty, ale w przyszłości może to być zachowanie domyślne dla `.csproj` pliku znajdującego się w tym samym folderze, w którym znajduje się plik notesu.</span><span class="sxs-lookup"><span data-stu-id="ee278-313">Currently this custom `<IQSharpLoadAutomatically>` property is required by :::no-loc(Q#)::: Jupyter Notebook hosts, but in the future, this may become the default behavior for a `.csproj` file located in the same folder as the notebook file.</span></span>

> [!NOTE]
> <span data-ttu-id="ee278-314">Obecnie `<QsharpCompile>` ustawienie w `.csproj` jest ignorowane przez :::no-loc(Q#)::: Jupyter Notebook hosty, a wszystkie `.qs` pliki w folderze `.csproj` (w tym podfolderach) są ładowane i kompilowane.</span><span class="sxs-lookup"><span data-stu-id="ee278-314">Currently the `<QsharpCompile>` setting in the `.csproj` is ignored by :::no-loc(Q#)::: Jupyter Notebook hosts, and all `.qs` files in the folder of the `.csproj` (including subfolders) are loaded and compiled.</span></span> <span data-ttu-id="ee278-315">Obsługa `.csproj` ustawień zostanie ulepszona w przyszłości (zobacz [iqsharp # 277](https://github.com/microsoft/iqsharp/issues/277) , aby uzyskać więcej informacji).</span><span class="sxs-lookup"><span data-stu-id="ee278-315">Support for `.csproj` settings will be improved in the future (see [iqsharp#277](https://github.com/microsoft/iqsharp/issues/277) for more details).</span></span>
