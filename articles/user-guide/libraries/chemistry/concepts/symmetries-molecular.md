...
Dwuwymiarowe części cząsteczkowe (tj. $h \_ {pq} $ i $h \_ {pqrs} $), takie jak te, są reprezentowane przy użyciu `OrbitalIntegral` typu, który zapewnia wiele przydatnych funkcji do wyrażania tego symetrii.
```csharp
...
Oprócz wyliczania wszystkich całków dwukierunkowych, które są identycznie takie same, lista wszystkich indeksów kręgosłupa zawartych w hamiltonian reprezentowanej przez `OrbitalIntegral` może być generowana w następujący sposób.
```csharp
...
Zamiast konstruowania Fermionic hamiltonian przez dodanie `FermionTerm` s, można automatycznie dodać wszystkie warunki odpowiadające każdej części dwuczęściowej.
